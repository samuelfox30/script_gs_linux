...
################################################################################
# Script de Automação - Servidor HTTP (VERSÃO COMPLETA)
...
# Definir IP fixo (usando o IP atual como base)
...
# Backup do arquivo original de configuração
cp /etc/netplan/*.yaml /etc/netplan/backup-netplan-$(date +%Y%m%d-%H%M%S).yaml 2>/dev/null || true

# Criar nova configuração de rede com IP fixo
...
log_info "Aplicando configuração de rede..."
netplan apply > /dev/null 2>&1
check_status "IP estático configurado com sucesso: $STATIC_IP" "Falha ao configurar IP estático"
...
log_info "Criando diretório temporário para download..."
mkdir -p /tmp/html_template

log_info "Baixando template HTML profissional da internet..."
# Template HTML5 responsivo e moderno
...
<body>
        ...
        <div class="hero-card">
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card" style="background: linear-gradient(135deg, var(--accent-color), #c0392b);">
                    <div class="stat-number">🔒</div>
                    <div class="stat-label">IP Fixo</div>
                </div>
            </div>
        </div>
        ...
        <div class="details-section">
            ...
            <ul class="details-list">
                ...
                <li>
                    ...
                    <span class="detail-value">IP Estático Configurado</span>
                </li>
                <li>
        ...
        <div class="details-section">
            ...
            <div class="info-grid">
                ...
                <div class="info-card">
                    ...
                    <div class="info-card-content">Firewall, versão oculta e IP fixo</div>
                </div>
    ...
    <script>
        ...
        function updateDateTime() {
            ...
            // Adicionar informações dinâmicas se necessário
...
Endereço IP         : $IP_ADDRESS (FIXO/ESTÁTICO)
...
Config. Rede        : /etc/netplan/01-netcfg.yaml
...
PORTAS EM ESCUTA
----------------
$(netstat -tlnp 2>/dev/null | grep apache2 || ss -tlnp | grep apache2)
...
✓ IP estático configurado ($STATIC_IP)
...
COMANDOS ÚTEIS
--------------
- Verificar status     : systemctl status apache2
- Reiniciar Apache     : systemctl restart apache2
- Verificar config     : apache2ctl configtest
- Ver logs em tempo real: tail -f /var/log/apache2/access.log
- Status do firewall   : ufw status verbose
- Testar conectividade : curl http://$IP_ADDRESS
...
echo "  ${BLUE}📍 Endereço IP:${NC}           $IP_ADDRESS ${GREEN}(FIXO/ESTÁTICO)${NC}"
...
echo "  ${GREEN}✓${NC} IP estático configurado"
